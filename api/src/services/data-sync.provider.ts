...
} from '../entities/core/last-block.entity'
import {
  getTokenList,
  PairEntity,
  savePair,
  updatePairReserves,
} from '../entities/core/pair.entity'
...
export class DataSyncProvider {
  ...
  public parseBlock = async (block: BlockDTO) => {
    ...
    try {
      const amm_state_changes = state.filter(
        s => s.key.split('.')[0] === config.amm_contract,
      )
      if (!amm_state_changes.length) return

      const currency_state_change = state.find(
        s =>
          s.key.split(':')[0] === 'currency.balances' &&
          s.key.split(':')[1] !== config.amm_contract,
      )
...
const saveTrade = async (state: I_Kvp[], hash, timestamp, user_vk: string) => {
  ...
  for (let token of traded_tokens) {
    ...
    const reserves = state.find(
      s => s.key.includes('reserves') && s.key.includes(contract_name),
    ).value
...
const processTrade = async (
  ...
  const pair_reserves_new = [
    Number(getValue(reserves[0])),
    Number(getValue(reserves[1])),
  ]

  const type: 'buy' | 'sell' =
    pair_reserves_old[0] < pair_reserves_new[0] ? 'buy' : 'sell'
